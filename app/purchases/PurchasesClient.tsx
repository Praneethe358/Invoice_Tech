...
export default function PurchasesClient({ shop, initialPurchases }: Props) {
  ...
  const fetchPurchases = useCallback(async (targetPage: number) => {
    ...
    let query = supabase
      .from('purchases')
      .select('id, shop_id, purchase_invoice_number, supplier_name, purchase_date, subtotal, total_gst, total_cgst, total_sgst, total, itc_eligible, created_at', { count: 'exact' })
      .eq('shop_id', shop.id)
    ...
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <PageTransition className="w-full px-4 md:px-8 pt-6 md:pt-0 pb-24">
        ...
        {purchases.length === 0 ? (
          ...
          <div className="md:bg-white md:border md:border-slate-200 md:shadow-xs md:rounded-2xl md:overflow-hidden bg-transparent border-none shadow-none">
            {/* Desktop Table View */}
            ...
            <div className="md:hidden flex flex-col gap-3.5">
              {purchases.map((p) => {
                const initials = p.supplier_name ? p.supplier_name.charAt(0).toUpperCase() : '?';
                const dateStr = new Date(p.purchase_date).toLocaleDateString('en-IN', {
                  day: '2-digit',
                  month: 'short',
                  year: 'numeric',
                ...
                return (
                  <div
                    key={p.id}
                    onClick={() => router.push(`/purchases/${p.id}`)}
                    className="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs active:scale-[0.99] transition-all duration-200 cursor-pointer flex flex-col gap-3"
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm border uppercase shadow-2xs ${
                          p.itc_eligible 
                        ...
                        <div>
                          <h4 className="text-sm font-bold text-slate-900 uppercase tracking-tight leading-tight">{p.supplier_name}</h4>
                          <span className="text-[10px] text-slate-400 font-semibold font-mono block mt-1">Invoice: #{p.purchase_invoice_number}</span>
                        </div>
                      </div>
                    ...
                    <div className="border-t border-slate-100 pt-2.5 flex items-center justify-between text-[11px] text-slate-500 font-medium">
                      <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-2 text-slate-400">
                        <span>Base: ₹{Number(p.subtotal).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center mt-1">
                      <div>
                        {p.itc_eligible ? (
                          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[9px] font-extrabold bg-emerald-50 text-emerald-700 border border-emerald-100 uppercase tracking-wider">
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                            ITC Eligible
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[9px] font-extrabold bg-slate-105 text-slate-500 border border-slate-200 uppercase tracking-wider">
                            No ITC Claim
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-1 text-[10px] font-bold text-blue-600 bg-blue-50/50 border border-blue-100/50 rounded-lg px-2.5 py-1.5 cursor-pointer">
                        <span>Details →</span>
                      </div>
                    </div>
                  </div>
