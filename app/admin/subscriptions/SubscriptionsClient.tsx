...
export default function SubscriptionsClient() {
  ...
  if (loading) return <div className="text-center py-20 text-slate-500 font-bold text-sm">Loading subscription datasets...</div>;
  if (!data) return <div className="text-center py-20 text-slate-500 font-bold text-sm">Failed to load subscription data</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-lg font-black text-white tracking-tight">Subscription Management</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <motion.div
            ...
            whileHover={{ y: -2 }}
            className={`bg-[#0b132b]/60 backdrop-blur-md border-t-2 border-x border-b border-slate-800/80 rounded-2xl p-5 shadow-lg hover:border-slate-700/80 transition-all duration-300 ${c.bg}`}
          >
            ...
            <p className="text-2xl font-black text-white tracking-tight">{c.v}</p>
          </motion.div>
        ))}
      </div>
      ...
      {data.renewals_due.length > 0 && (
        <div className="bg-[#0b132b]/40 backdrop-blur-md border border-slate-800 rounded-2xl shadow-lg overflow-hidden">
            ...
            <tbody className="divide-y divide-slate-800/40">
              {data.renewals_due.map(shop => (
                <tr key={shop.id} className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-4 px-5 font-bold text-white cursor-pointer hover:text-blue-400" onClick={() => router.push(`/admin/shops/${shop.id}`)}>{shop.name}</td>
                  <td className="py-4 px-5 text-slate-300 font-semibold">{formatDate(shop.subscription_ends_at)}</td>
                  <td className="py-4 px-5">
                    <span className={`inline-flex px-2.5 py-0.5 rounded-full text-[9px] font-black border ${
                      (shop.days_left || 0) <= 3 
                        ? 'bg-rose-500/10 text-rose-400 border-rose-500/25 shadow-[0_0_15px_rgba(244,63,94,0.15)]' 
                        : (shop.days_left || 0) <= 7 
                        ? 'bg-amber-500/10 text-amber-400 border-amber-500/25' 
                        : 'bg-slate-800/60 text-slate-400 border-slate-700/65'
                    }`}>{shop.days_left} days</span>
                  </td>
                  <td className="py-4 px-5">
                    <button onClick={() => setActivatingId(activatingId === shop.id ? null : shop.id)}
                      className="px-3.5 py-1.5 rounded-xl text-[10px] font-black bg-blue-500/10 text-blue-400 border border-blue-500/20 hover:bg-blue-500/25 transition-all duration-300">Extend</button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <table className="w-full text-xs text-left">
            ...
            <tbody className="divide-y divide-slate-800/40">
              {data.trials_ending_soon.map(shop => (
                <tr key={shop.id} className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-4 px-5 font-bold text-white cursor-pointer hover:text-blue-400" onClick={() => router.push(`/admin/shops/${shop.id}`)}>{shop.name}</td>
                  <td className="py-4 px-5 text-slate-300 font-semibold">{formatDate(shop.trial_ends_at)}</td>
                  <td className="py-4 px-5">
                    <span className={`inline-flex px-2.5 py-0.5 rounded-full text-[9px] font-black border ${
                      (shop.days_left || 0) <= 3 
                        ? 'bg-rose-500/10 text-rose-400 border-rose-500/25 shadow-[0_0_15px_rgba(244,63,94,0.15)]' 
                  ...
                  <td className="py-4 px-5">
                    <button onClick={() => setActivatingId(activatingId === shop.id ? null : shop.id)}
                      className="px-3.5 py-1.5 rounded-xl text-[10px] font-black bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:from-blue-500 hover:to-indigo-500 shadow-md shadow-blue-500/10 transition-all duration-300">Activate</button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <table className="w-full text-xs text-left">
            ...
            <tbody className="divide-y divide-slate-800/40">
              {data.expired_this_month.map(shop => (
                <tr key={shop.id} className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-4 px-5 font-bold text-white">{shop.name}</td>
                  <td className="py-4 px-5">
                    <span className={`inline-flex px-2.5 py-0.5 rounded-full text-[9px] font-black border capitalize ${STATUS_COLORS[shop.subscription_status]}`}>
                      {shop.subscription_status}
                    </span>
                  </td>
                  <td className="py-4 px-5">
                    <button onClick={() => setActivatingId(activatingId === shop.id ? null : shop.id)}
                      className="px-3.5 py-1.5 rounded-xl text-[10px] font-black bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:from-blue-500 hover:to-indigo-500 shadow-md shadow-blue-500/10 transition-all duration-300">Reactivate</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {activatingId && (
        <motion.div 
          initial={{ opacity: 0, y: 8 }} 
          animate={{ opacity: 1, y: 0 }}
          className="bg-slate-950/60 border border-slate-800 rounded-2xl p-5 shadow-xl border-l-4 border-l-blue-500"
        >
          <p className="text-xs font-bold text-slate-200 mb-3">Select duration:</p>
          ...
          <div className="flex items-end gap-3">
            ...
            <button 
              ...
              className="px-5 py-2.5 rounded-xl text-xs font-black bg-gradient-to-r from-blue-600 to-indigo-600 text-white disabled:opacity-50 transition-all duration-300 shadow-md shadow-blue-500/10"
            >
