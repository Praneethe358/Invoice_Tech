...
export default function SuppliersClient({ shop, initialSuppliers }: Props) {
  ...
  const fetchSuppliers = useCallback(async (targetPage: number) => {
    ...
    let query = supabase
      .from('suppliers')
      .select('id, shop_id, name, phone, address, gstin, created_at', { count: 'exact' })
      .eq('shop_id', shop.id)
        ...
        {suppliers.length === 0 ? (
          ...
          <div className="md:bg-white md:border md:border-slate-200 md:shadow-xs md:rounded-2xl md:overflow-hidden bg-transparent border-none shadow-none">
            ...
            <div className="md:hidden flex flex-col gap-3.5">
              {suppliers.map((sup) => {
                const initials = sup.name ? sup.name.charAt(0).toUpperCase() : '?';
                const hasGst = !!sup.gstin;
                return (
                  <div
                    key={sup.id}
                    onClick={() => router.push(`/suppliers/${sup.id}`)}
                  ...
                  >
                      ...
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm border uppercase shadow-2xs ${
                          hasGst 
                            ...
                            : 'bg-[#0050e8]/5 border-[#0050e8]/10 text-[#0050e8]'
                        }`}>
                          {initials}
                        </div>
                        <div>
                          <h4 className="text-sm font-bold text-slate-900 uppercase tracking-tight leading-tight">{sup.name}</h4>
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider mt-1 border ${
                            hasGst 
                              ? 'bg-emerald-50 text-emerald-700 border-emerald-100' 
                              : 'bg-slate-100 text-slate-500 border-slate-200'
                          }`}>
                            {hasGst ? `B2B · ${sup.gstin}` : 'Unregistered'}
                          </span>
                        </div>
                      </div>
                      <div className="text-right">
                        <span className="text-[10px] font-bold text-slate-400 block uppercase tracking-wider">Purchases</span>
                    ...
                    {(sup.phone || sup.address) && (
                      <div className="border-t border-slate-100 pt-2.5 flex flex-col gap-1.5 text-[11px] text-slate-500 font-medium">
                        {sup.phone && (
                          <div className="flex items-center gap-2">
                            <svg className="w-3.5 h-3.5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                              <path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.94.725l.548 2.2a1 1 0 01-.321.988l-1.305.98a10.582 10.582 0 004.872 4.872l.98-1.305a1 1 0 01.988-.321l2.2.548a1 1 0 01.725.94V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                            </svg>
                            <span>+91 {sup.phone.slice(-10)}</span>
                          </div>
                        )}
                        {sup.address && (
                          <div className="flex items-center gap-2">
                    ...
                    <div className="flex justify-between items-center text-[10px] font-bold text-blue-600 bg-blue-50/50 border border-blue-100/50 rounded-xl px-3 py-2 mt-1">
